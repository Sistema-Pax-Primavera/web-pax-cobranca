...
const EscritorioCobranca = () => {
        ...
        clientesFiltrados = clientesFiltrados.filter((cliente) => {
          if (cliente.dados_cliente.observacao) {
            return cliente.dados_cliente.observacao.some((obs) => {
              const dataUltJustificativa = new Date(
                converterDataParaFormatoBackend(obs.data_criacao)
              );
        ...
        clientesFiltrados = clientesFiltrados.filter((cliente) => {
          if (cliente.dados_cliente.agendamentos) {
            return cliente.dados_cliente.agendamentos.some((agendamento) => {
              const dataUltJustificativa = new Date(
                converterDataParaFormatoBackend(agendamento.ult_justificativa)
              );
              ...
              return (
                agendamento.id_justificativa === idJustFormat &&
  ...
  return (
    <div className="container-cobranca-escritorio">
                ...
                conteudo={
                  <div className="container-modal-lateral">
                    ...
                    <div className="campos-filtro">
                      ...
                      <Checkbox
                        checked={isAgendemento}
                        onChange={(e) => setIsAgendamento(e.target.checked)}
                      />
                      <label>Sem justificativa</label>
                      <Checkbox
                        checked={isSemJustificativa}
                        onChange={(e) =>
                          setIsSemJustificativa(e.target.checked)
                        }
                      />
                    </div>
                    <div className="campos-filtro">
                    ...
                    <div className="campos-filtro">
                      ...
                      <Checkbox
                        checked={isJustificativa}
                        onChange={(e) => setIsJustificativa(e.target.checked)}
                      />
                      ...
                      <div className="campos-filtro">
                        <div>
                          ...
                          >
                            ...
                            <option value={3}>
                              TITULAR NÃO ESTAVA EM CASA
                            </option>
                            <option value={4}>AGENDOU PARA:</option>
                            <option value={5}>SOLICITOU CANCELAMENTO</option>
                            <option value={6}>IRÁ AO ESCRITORIO</option>
                            <option value={7}>
                              LIGARA P/ COB OU ESC QUANDO FOR PAGAR
                            </option>
                            <option value={8}>
                              TRANSFERIR PARA OUTRA ROTA
                            </option>
                            <option value={9}>
                              EFETUA PAGAMENTO ANTECIPADO
                            </option>
                            <option value={10}>EM VIAGEM</option>
                            <option value={11}>
                              ENDEREÇO RES. E COM. NÃO CORRESPONDE AO CLIENTE
                            </option>
      ...
      {isLoading ? (
        <div className="alinhando-loading">
          <Carregando />
        </div>
      ) : (
